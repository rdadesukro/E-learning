<?php
	...
	if (empty($id)) { 
		...
		$actualpath = "$path";
		$num_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM user WHERE id='".$id."'"));
		
		
	       if ($num_rows == 0){
		 	    $query = mysqli_query($con, "INSERT INTO user (nama,alamat,no_hp,email) VALUES ('$nama','$alamat','$no_hp','$email')");
				if ($query){
				...
				$query2 = mysqli_query($con, "UPDATE user SET nama='$nama',alamat='$alamat',no_hp='$no_hp',email='$email' WHERE id='$id'");
