...
/*	include 'koneksi.php';
	...
	if ((empty($email))) { 
		...
		if (!empty($email) && $password == $confirm_password){
			$num_rows = mysql_num_rows(mysql_query("SELECT * FROM user WHERE email='".$email."'"));

			if ($num_rows == 0){
				$query = mysql_query("INSERT INTO user (id, email, password) VALUES(0,'".$email."','".$password."')");
 ...
 $confirm_password = md5($_POST["confirm_password"]);
	 if ((empty($email))) {
		...
		die(json_encode($response));
		 ...
		 if (!empty($email) && $password == $confirm_password){
		 	$num_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM user WHERE email='".$email."' and kunci='".$kunci."'"));
