...
public class DataParser_mapel extends AsyncTask<Void,Void,Boolean> {
    ...
    public DataParser_mapel(Context c, String jsonData, RecyclerView rv, SwipeRefreshLayout swipeRefreshLayout) {
        this.c = c;
    ...
    protected void onPostExecute(Boolean parsed) {
        ...
        if(parsed)
        {
            ...
            MyAdapter_mapel adapter = new MyAdapter_mapel(c, objekDatas);
    ...
    private Boolean parseData()
    {
        try
        {
            ...
            for (int i=0;i<ja.length();i++)
            {
                ...
                String id=jo.getString("id_mapel");
                String nm=jo.getString("nama_mapel");
                String ft=jo.getString("foto_mapel");
